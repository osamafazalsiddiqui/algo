...
function addRow() {
  ...
  var minus = $('#minus');
  minus.show();
  minus.css('top', (parseFloat(minus.css('top')) + 29) + 'px');
...
function animationStep(steps, cur) {
	$('#timer, #additionalId').html(cur); // Add the additional ID here
	if (cur < steps) {
		setTimeout(function () {
			animationStep(steps, cur + 1);
		}, 500);
	} else {
		// Any additional logic when the animation is complete
	}
}
